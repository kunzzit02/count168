...
添加一行（每月 4 号 16:00 执行，请把路径改成你服务器上的实际路径）：

```cron
0 16 4 * * /usr/bin/php /var/www/count168/auto_monthly_accounting.php >> /var/www/count168/auto_accounting_log.txt 2>&1
```

说明：`0 16 4 * *` = 每月 4 号 16:00。

---

## 三、今天 4 号但还没跑过？手动补跑

在项目目录下执行（例如今天就是 4 号、已经过了 16:00 但 Transaction 仍没数据时）：
