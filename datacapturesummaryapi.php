...
function ensureTemplateSchema(PDO $pdo) {
    ...
    try {
        ...
        // Ensure sub_order column exists to preserve sub row ordering
        try {
...
function fetchTemplates(PDO $pdo, array $ids, ?int $processId = null) {
    ...
    $stmt = $pdo->prepare("
        ...
        ORDER BY process_id DESC,
                 ...
                 row_index ASC,
                 product_type ASC,
                 CASE WHEN product_type = 'sub' AND sub_order IS NOT NULL THEN 0 ELSE 1 END,
                 sub_order ASC,
                 formula_variant ASC,
                 id ASC
    ");
...
if ($action === 'save_template' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $templatePayload = [
            ...
            // Preserve sub row ordering if provided
            'sub_order' => isset($row['sub_order']) && $row['sub_order'] !== null && $row['sub_order'] !== '' ? (float)$row['sub_order'] : null,
