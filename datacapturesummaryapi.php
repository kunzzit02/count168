...
if ($action === 'submit' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        try {
            ...
            // 辅助函数：去除序号前缀（如果存在）
            $removePrefix = function($idProduct) {
                if (preg_match('/^\d+\.\s(.+)$/', $idProduct, $matches)) {
                    return trim($matches[1]);
                }
                return trim($idProduct);
            };
            ...
            // 第一遍遍历：统计每个 id product 出现的次数（去除前缀后统计）
            foreach ($data['summaryRows'] as $row) {
                $idProductMain = trim($row['idProductMain'] ?? '');
                $idProductSub = trim($row['idProductSub'] ?? '');
                
                if (!empty($idProductMain)) {
                    $idProductMainNormalized = $removePrefix($idProductMain);
                    if (!isset($idProductMainCounts[$idProductMainNormalized])) {
                        $idProductMainCounts[$idProductMainNormalized] = 0;
                    }
                    $idProductMainCounts[$idProductMainNormalized]++;
                }
                
                if (!empty($idProductSub)) {
                    $idProductSubNormalized = $removePrefix($idProductSub);
                    if (!isset($idProductSubCounts[$idProductSubNormalized])) {
                        $idProductSubCounts[$idProductSubNormalized] = 0;
                    }
                    $idProductSubCounts[$idProductSubNormalized]++;
                }
            }
            
            // 第二遍遍历：重命名重复的 id product
            // 使用计数器来跟踪每个 id product 当前是第几个出现
            $idProductMainCurrentCount = [];
            $idProductSubCurrentCount = [];
            ...
            foreach ($data['summaryRows'] as &$row) {
                ...
                if (!empty($idProductMain)) {
                    ...
                    // 检查是否有重复（出现次数 > 1）
                    if (isset($idProductMainCounts[$idProductMainNormalized]) && $idProductMainCounts[$idProductMainNormalized] > 1) {
                        // 初始化计数器
                        if (!isset($idProductMainCurrentCount[$idProductMainNormalized])) {
                            $idProductMainCurrentCount[$idProductMainNormalized] = 0;
                        }
                        
                        // 增加计数器
                        $idProductMainCurrentCount[$idProductMainNormalized]++;
                        
                        // 检查是否已经有前缀（避免重复添加）
                        $hasPrefix = preg_match('/^\d+\.\s/', $idProductMain);
                        
                        if (!$hasPrefix) {
                            // 添加序号前缀
                            $row['idProductMain'] = $idProductMainCurrentCount[$idProductMainNormalized] . '. ' . $idProductMainNormalized;
                            error_log("Renamed duplicate idProductMain: '$idProductMain' -> '{$row['idProductMain']}'");
                        }
                    }
                }
                ...
                if (!empty($idProductSub)) {
                    ...
                    // 检查是否有重复（出现次数 > 1）
                    if (isset($idProductSubCounts[$idProductSubNormalized]) && $idProductSubCounts[$idProductSubNormalized] > 1) {
                        // 初始化计数器
                        if (!isset($idProductSubCurrentCount[$idProductSubNormalized])) {
                            $idProductSubCurrentCount[$idProductSubNormalized] = 0;
                        }
                        
                        // 增加计数器
                        $idProductSubCurrentCount[$idProductSubNormalized]++;
                        
                        // 检查是否已经有前缀（避免重复添加）
                        $hasPrefix = preg_match('/^\d+\.\s/', $idProductSub);
                        
                        if (!$hasPrefix) {
                            // 添加序号前缀
                            $row['idProductSub'] = $idProductSubCurrentCount[$idProductSubNormalized] . '. ' . $idProductSubNormalized;
                            error_log("Renamed duplicate idProductSub: '$idProductSub' -> '{$row['idProductSub']}'");
                        }
                    }
                }
            }
