    ...
    return null;
        ...
        function handleFormulaValueInput(formulaInput, value) {
            ...
            if (/^\d$/.test(value)) {
                ...
                // 没有选中行或没找到列，就按普通数字插入
                if (!formulaInput) {
                    console.warn('formulaInput is null, cannot proceed');
                    return;
                }
                const textAfter = formulaInput.value.substring(formulaInput.selectionEnd || cursorPos);
                ...
                if (formulaInput.setSelectionRange) {
                    formulaInput.setSelectionRange(newCursorPos, newCursorPos);
                }
                if (formulaInput.focus) {
                    formulaInput.focus();
        ...
        function updateFormulaDataGrid() {
            ...
            // IMPORTANT: Get row_label or data-row-index from currentSelectedRowForCalculator
            // This ensures we only show data from the current editing row, not all rows with same id_product
            ...
            rows.forEach((row, rowIndex) => {
                ...
                } else if (targetRowIndex !== null) {
                    // If row_label not available but targetRowIndex is, use row index to match
                    if (rowIndex !== targetRowIndex) {
                        console.log('updateFormulaDataGrid - Skipping row with different row_index:', rowIndex, 'expected:', targetRowIndex);
                        return; // Skip this row if row index doesn't match
                    }
                    console.log('updateFormulaDataGrid - Found matching row with row_index:', targetRowIndex);
                }
        ...
        function insertCellValueToFormula(cell) {
            ...
            // Double-check formulaInput is still available
            if (!formulaInput) {
                console.warn('formulaInput is null, cannot proceed');
                return;
            }
