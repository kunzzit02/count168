...
function reorderSummaryRowsByRowIndex() {
    try {
        ...
        // For rows without row_index, try to calculate row_index from Data Capture Table position
        // This ensures rows with same id_product are ordered correctly based on Data Capture Table
        const capturedTableBody = document.getElementById('capturedTableBody');
        if (capturedTableBody) {
            const capturedRows = Array.from(capturedTableBody.querySelectorAll('tr'));
            const usedIndices = new Set(rowData.map(r => r.rowIndex).filter(idx => idx !== null));
            
            // First pass: Try to match rows with account_id and formula_variant for more precise matching
            rowData.forEach(rowDataItem => {
                if (rowDataItem.rowIndex === null) {
                    const idProductCell = rowDataItem.row.querySelector('td:first-child');
                    const productValues = getProductValuesFromCell(idProductCell);
                    const mainTextRaw = (productValues.main || '').trim();
                    const normalizedMain = normalizeIdProductText(mainTextRaw);
                    
                    if (normalizedMain) {
                        // Get account_id and formula_variant from row for precise matching
                        const accountCell = rowDataItem.row.querySelector('td:nth-child(2)');
                        const rowAccountId = accountCell?.getAttribute('data-account-id');
                        const rowFormulaVariant = rowDataItem.row.getAttribute('data-formula-variant');
                        
                        // Try to find matching row in Data Capture Table with same account_id and formula_variant
                        let matched = false;
                        if (rowAccountId || rowFormulaVariant) {
                            for (let i = 0; i < capturedRows.length; i++) {
                                if (usedIndices.has(i)) continue;
                                
                                const capturedRow = capturedRows[i];
                                const capturedIdProductCell = capturedRow.querySelector('td[data-column-index="1"]') || 
                                                              capturedRow.querySelector('td[data-col-index="1"]') || 
                                                              capturedRow.querySelectorAll('td')[1];
                                if (capturedIdProductCell) {
                                    const capturedIdProduct = normalizeIdProductText(capturedIdProductCell.textContent.trim());
                                    if (capturedIdProduct === normalizedMain) {
                                        // Found matching id_product, check if we can verify account_id match
                                        // (Note: Data Capture Table doesn't have account_id, so we can't verify)
                                        // Use this position if it matches id_product
                                        rowDataItem.rowIndex = i;
                                        rowDataItem.row.setAttribute('data-row-index', String(i));
                                        usedIndices.add(i);
                                        matched = true;
                                        break;
                                    }
                        ...
                        // Second pass: If not matched, find any matching id_product position
                        if (!matched) {
                            for (let i = 0; i < capturedRows.length; i++) {
                                if (usedIndices.has(i)) continue;
                                ...
                                if (capturedIdProductCell) {
                                    const capturedIdProduct = normalizeIdProductText(capturedIdProductCell.textContent.trim());
                                    if (capturedIdProduct === normalizedMain) {
                                        // Found matching row in Data Capture Table, use its position as row_index
                                        rowDataItem.rowIndex = i;
                                        rowDataItem.row.setAttribute('data-row-index', String(i));
                                        usedIndices.add(i);
                                        break;
        ...
        // Now separate rows with and without row_index
