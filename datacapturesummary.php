    ...
    return null;
        ...
        function addSubIdProductRow(parentProcessValue, insertAfterRow = null, rowIndex = null) {
            ...
            if (insertAfterIndex >= 0) {
                ...
                // Calculate sub_order based on insertion position
                // Use decimals (0.1, 0.2, etc.) when inserting between rows to maintain exact position
                if (insertAfterRow) {
                    ...
                    if (insertAfterProductType === 'sub') {
                        ...
                        if (nextRow) {
                            ...
                            if (nextProductType === 'sub') {
                                ...
                                if (insertAfterSubOrder !== null && !isNaN(insertAfterSubOrder) && 
                                    ...
                                    // Next row has no sub_order, use insertAfterSubOrder + 0.1
                        ...
                        if (foundSubRow && minSubOrder !== null && minSubOrder > 0) {
                            ...
                            // No sub rows found after insertion position: use 0.1 (first sub row before integer ones)
                            subOrder = 0.1;
                        }
                    }
                } else {
                    // Appending: use maxSubOrder + 1 (integer if max is integer, otherwise use integer part + 1)
                    if (maxSubOrder > 0) {
                        if (maxSubOrder % 1 === 0) {
                            // maxSubOrder is an integer
                            subOrder = maxSubOrder + 1;
                        } else {
                            // maxSubOrder is a decimal, use integer part + 1
                            subOrder = Math.floor(maxSubOrder) + 1;
                        }
                    } else {
                        subOrder = 1;
                    }
                ...
                // Find all sub rows for the same parent and use max sub_order + 1 (integer if max is integer, otherwise use integer part + 1)
                let subOrder = 1;
                const allRowsBeforeAppend = summaryTableBody.querySelectorAll('tr');
                let maxSubOrder = 0;
                ...
                // If maxSubOrder is an integer, use maxSubOrder + 1; otherwise use Math.floor(maxSubOrder) + 1
                if (maxSubOrder > 0) {
                    if (maxSubOrder % 1 === 0) {
                        // maxSubOrder is an integer
                        subOrder = maxSubOrder + 1;
