...
function getCurrentProcessId() {
    ...
    return null;
        ...
        function showEditFormulaForm(productValue, isSubIdProduct = false, prePopulatedData = null) {
            ...
            } else if (prePopulatedData && prePopulatedData.rowLabel) {
                // Store row label from prePopulatedData
                window.currentEditRowLabel = prePopulatedData.rowLabel;
            }
        ...
        function editRowFormula(button) {
                ...
                if (capturedTableBody) {
                    ...
                    capturedRows.forEach((capturedRow, capturedRowIndex) => {
                        ...
                        if (rowIdProduct && rowIdProduct.trim() === processValue.trim()) {
                            ...
                            const rowLabel = rowHeaderCell ? rowHeaderCell.textContent.trim() : '';
                            if (rowLabel) {
                                // Check if this row's data is used in the current summary row
                                // by checking if any column from this captured row matches the sourceColumnsValue
                                const cells = capturedRow.querySelectorAll('td');
                                let hasMatchingColumn = false;
                                
                                cells.forEach((cell) => {
                                    const columnIndex = cell.getAttribute('data-column-index');
                                    if (columnIndex && parseInt(columnIndex) > 1) {
                                        // Check if this column is in the sourceColumnsValue
                                        const columnPattern = new RegExp(`:${columnIndex}(?:\\s|$)`);
                                        if (sourceColumnsValue.match(columnPattern)) {
                                            hasMatchingColumn = true;
                                        }
                                    }
                                });
                                
                                // If this row has matching columns, use its row label
                                if (hasMatchingColumn && !currentEditRowLabel) {
            ...
            showEditFormulaForm(processValue, false, {
                ...
                rowLabel: currentEditRowLabel // Pass row label to filter data
            });
        }
