 ...
 * Win/Loss = Data Capture + 所有 Bank Process 的 WIN/LOSE（Remaining days / 1号 / Monthly 均在此列）
 * Bank Process 以 description LIKE 'Process:%' 识别
 */
function calculateWinLossByCurrency($pdo, $account_id, $currency_id, $date_from, $date_to, $company_id) {
    ...
    // 2. 所有 Bank Process 的 WIN/LOSE 计入 Win/Loss（Remaining / 1号 / Monthly；Cost/Profit 左+，Sell Price 右-）
    $stmt = $pdo->query("SHOW COLUMNS FROM transactions LIKE 'currency_id'");
    if ($stmt->rowCount() > 0) {
...
function calculateCrDrByCurrency($pdo, $account_id, $currency_id, $date_from, $date_to, $company_id) {
    ...
    if ($has_currency_id) {
        // Cr/Dr = PAYMENT/RECEIVE/CONTRA/CLAIM + 非 Bank Process 的 WIN/LOSE（Bank Process 全部在 Win/Loss 列）
        $sql = "
            SELECT
                COALESCE(SUM(
                    CASE
                        -- 作为 To Account（收到 / 支付 / Win/Lose；排除 description LIKE 'Process:%' 的 Bank Process）
        ...
        // 旧环境（没有 currency_id 字段）：Cr/Dr 排除 Bank Process（description LIKE 'Process:%'）
        $sql = "SELECT 
                    COALESCE(SUM(CASE 
                        WHEN transaction_type IN ('RECEIVE', 'CONTRA', 'CLAIM') THEN t.amount
                        WHEN transaction_type = 'PAYMENT' THEN t.amount
