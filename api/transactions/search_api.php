...
function calculateCrDrByCurrency($pdo, $account_id, $currency_id, $date_from, $date_to, $company_id) {
    ...
    if ($has_currency_id) {
        // Cr/Dr = PAYMENT/RECEIVE/CONTRA/CLAIM/CLEAR；WIN/LOSE（含 PROFIT）计入 Win/Loss 列
        $sql = "
            SELECT
                COALESCE(SUM(
                    CASE
        ...
        // 旧环境（没有 currency_id 字段）：Cr/Dr 仅 PAYMENT/RECEIVE/CONTRA/CLAIM/CLEAR；WIN/LOSE 计入 Win/Loss
        $sql = "SELECT 
                    COALESCE(SUM(CASE 
