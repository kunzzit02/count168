...
try {
    ...
    foreach ($transactions as $t) {
        ...
        switch ($t['transaction_type']) {
            case 'WIN':
                // 与 CONTRA 一致：To Account +xxx.xx，From Account -xxx.xx
        ...
        // WIN/LOSE（前端以 PROFIT 提交）：与 CONTRA 同格式；To Account 显示 PROFIT FROM {From}，From Account 显示 PROFIT TO {To}
        if (in_array($t['transaction_type'], ['WIN', 'LOSE'])) {
            if ($is_to_account) {
                $description = 'PROFIT FROM ' . ($t['from_account_code'] ?: $t['to_account_code'] ?: 'N/A');
            } else {
                $description = 'PROFIT TO ' . ($t['to_account_code'] ?: 'N/A');
            }
        }
...
function calculateBF($pdo, $account_id, $date_from, $company_id) {
    ...
    // PAYMENT/RECEIVE/CONTRA/CLAIM/RATE/WIN/LOSE 影响 Cr/Dr（作为 From Account）
    // WIN/LOSE（PROFIT）：From Account 与 CONTRA 一致，WIN 为 -amount，LOSE 为 +amount
    $sql = "SELECT 
                COALESCE(SUM(CASE 
