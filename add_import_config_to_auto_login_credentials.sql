...
ADD COLUMN `report_page_url` VARCHAR(500) NULL COMMENT '报告页面URL（如果与登录URL不同，用于网页抓取模式）' AFTER `auto_import_enabled`,
ADD COLUMN `import_process_id` INT(11) NULL COMMENT '导入流程ID（关联process表）' AFTER `report_page_url`,
