    ...
    <script>
        ...
        function handleCellPaste(e) {
            ...
            if (typeof currentDataCaptureType !== 'undefined' && currentDataCaptureType === 'AWC') {
                ...
                // 方法4：智能解析单列格式（每行一个单元格，但实际是行优先格式）
                if (!formatDetected) {
                    console.log('AWC (2.7): Trying smart parsing for single-column row-major format...');
                    const normalizedData = pastedData.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
                    const lines = normalizedData.split('\n').map(line => line.trim()).filter(line => line !== '');
                    
                    if (lines.length > 10) { // 确保有足够的数据
                        // 提取所有单元格
                        ...
                        // 排除列表：常见的非用户ID标识
                        const excludeList = ['LIVE', 'TABLE', 'SV388', 'SEXYBCRT', 'KINGMIDAS', 'VICTORBETVTB'];
                        ...
                        for (let i = 0; i < allCells.length; i++) {
                            ...
                            // 用户ID通常是：以小写字母开头，2-15个字符，主要是字母和数字
                            if (userIDPattern.test(cell) && 
                                !excludeList.includes(upperCell) &&
                                cell.length >= 2 && cell.length <= 15) {
                                // 进一步验证：
                                // 1. 必须以小写字母开头（如op7a, tr8）
                                // 2. 或者混合大小写但主要是小写（如victorbetvtb）
                                // 3. 不能是纯数字
                                // 4. 不能是常见的平台或类型名称
                                if ((cell.match(/^[a-z]/) || cell.match(/^[a-z][a-z0-9]*$/i)) && 
                                    !cell.match(/^\d+$/) &&
                                    !cell.match(/^(LIVE|TABLE|SV\d+|SEXY|KING|VICTOR)$/i)) {
                                    rowMarkers.push({ index: i, type: 'userID', value: cell });
                                }
                            }
                            
                            // 识别Sub Total标记（可能包含用户ID，如"Sub Total[ op7a ]"）
                            if (subTotalPattern.test(cell)) {
                                rowMarkers.push({ index: i, type: 'subTotal', value: cell });
                            }
                        }
                        
                        console.log(`AWC (2.7): Found ${rowMarkers.length} row markers:`, rowMarkers.slice(0, 15));
                        
                        // 方法1：使用Sub Total和下一个用户ID之间的间隔来确定列数
                        ...
                        if (subTotalMarkers.length > 0 && userIDMarkers.length > 0) {
                            // 找到每个Sub Total后面紧跟的下一个用户ID
                            const intervals = [];
                            for (const subTotal of subTotalMarkers) {
                                // 找到这个Sub Total之后最近的一个用户ID
                                const nextUserID = userIDMarkers.find(u => u.index > subTotal.index);
                                if (nextUserID) {
                                    const interval = nextUserID.index - subTotal.index;
                                    // Sub Total通常在一行的末尾，下一个用户ID在下一行的开头
                                    // 所以间隔就是每行的列数
                            ...
                            if (intervals.length > 0) {
                                ...
                                console.log(`AWC (2.7): Sub Total to next User ID intervals:`, intervals);
                        ...
                        // 方法3：如果还是没找到，尝试常见的列数（18列最常见）
                        if (!detectedColumns) {
                            // 尝试18列，因为这是最常见的列数
                            const testColumns = 18;
                            const testRows = Math.ceil(allCells.length / testColumns);
                            const remainder = allCells.length % testColumns;
                            
                            // 如果能整除或剩余很少，使用18列
                            if (remainder === 0 || remainder / testColumns < 0.1) {
                                detectedColumns = testColumns;
                                console.log(`AWC (2.7): Using default 18 columns (${testRows} rows, remainder: ${remainder})`);
                            }
                        }
