        ...
        function handleCellPaste(e) {
            ...
            if (typeof currentDataCaptureType !== 'undefined' && currentDataCaptureType === '2.SPECIAL') {
                ...
                if (!formatDetected) {
                    ...
                    console.log('2.SPECIAL: ALIPAY mode detected, attempting to parse...');
                    console.log('Pasted data length:', pastedData.length);
                    ...
                    if (htmlDataFromDetect) {
                        console.log('2.SPECIAL: ALIPAY format - HTML data detected via detectAndParseHTML');
                        const filled = parseAndFillHTMLTable(htmlDataFromDetect, startCell);
                        if (filled) {
                            ...
                            console.log('2.SPECIAL: ALIPAY format - parseAndFillHTMLTable returned false, trying manual HTML parsing');
                        }
                    }
                    ...
                    try {
                        ...
                        console.log('2.SPECIAL: ALIPAY format - Could not get HTML data from clipboard:', err);
                    }
                    
                    if (htmlData) {
                        console.log('2.SPECIAL: ALIPAY format - HTML data detected, length:', htmlData.length);
                        // 解析 HTML 表格，保持原始格式
                        try {
                            const tempDiv = document.createElement('div');
                            tempDiv.innerHTML = htmlData;
                            ...
                            const table = tempDiv.querySelector('table');
                            if (table) {
                                console.log('2.SPECIAL: ALIPAY format - HTML table found');
                                let dataMatrix = [];
                                ...
                                if (dataMatrix.length > 0) {
                                    ...
                                    console.log('2.SPECIAL: ALIPAY format - HTML parsing successful -', dataMatrix.length, 'rows x', maxCols, 'cols');
                                    ...
                                    console.log('2.SPECIAL: ALIPAY format - HTML table found but no data rows extracted');
                                }
                            } else {
                                console.log('2.SPECIAL: ALIPAY format - HTML data exists but no table element found');
                            }
                        } catch (htmlErr) {
                            console.error('2.SPECIAL: ALIPAY format - HTML parser error:', htmlErr);
                        }
                    } else {
                        console.log('2.SPECIAL: ALIPAY format - No HTML data detected, will try text parsing');
                    }
                    
                    // 如果 HTML 解析失败，尝试纯文本解析
                    if (!alipayParsed) {
                        console.log('2.SPECIAL: ALIPAY format - Attempting text format parsing...');
                        const normalizedData = pastedData.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
                        const lines = normalizedData.split('\n').map(line => line.trim()).filter(line => line !== '');
                        ...
                        if (lines.length > 0) {
                            ...
                            if (lines.length >= 3) {
                                ...
                                if (identifierCount >= 2 && nameLikeLineCount >= identifierCount * 0.5) {
                                    ...
                                    console.log('2.SPECIAL: ALIPAY format - Detected Name column format (', nameLikeLineCount, 'out of', identifierCount, 'identifiers)');
                                }
                            }
                                                    ...
                                                    if (isNameLike) {
                                                        ...
                                                        if (i + 2 < lines.length) {
                                                            ...
                                                            if (hasNumbers) {
                                                                ...
                                                                // 跳过 Name 行的处理
                                                                i++; // 跳过下一行（Name 行）
                                                                console.log('2.SPECIAL: ALIPAY format - Detected Name column value:', nameValue, 'for identifier:', trimmedLine);
                                                            }
                                                        }
                                                    }
                                                }
                            ...
                            if (dataMatrix.length > 0) {
                                console.log('2.SPECIAL: ALIPAY format - Text parsing successful -', dataMatrix.length, 'rows x', maxCols, 'cols');
                                console.log('2.SPECIAL: ALIPAY format - First row sample:', dataMatrix[0] ? dataMatrix[0].slice(0, 10) : 'empty');
                                alipayParsed = {
                                    dataMatrix: dataMatrix,
                                    maxRows: dataMatrix.length,
                                    maxCols: maxCols
                    ...
                    if (alipayParsed) {
                        ...
                        if (successCount > 0) {
                            ...
                            console.log('2.SPECIAL: ALIPAY format - No cells were pasted');
                        }
                    } else {
                        // ALIPAY 格式解析失败，继续尝试其他格式
                        console.log('2.SPECIAL: ALIPAY format - parser returned null, data may not match expected format, continuing with other formats');
                    }
                }
