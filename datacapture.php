    ...
    <script>
        ...
        function handleCellPaste(e) {
            ...
            if (typeof currentDataCaptureType !== 'undefined' && currentDataCaptureType === '2.SPECIAL') {
                ...
                // ===== 2.2 VPOWER 格式检测和处理（检测表头包含 User Name 和 profit） =====
                if (!formatDetected) {
                    console.log('2.SPECIAL: Trying 2.2 VPOWER format...');
                    console.log('2.SPECIAL: VPOWER - Pasted data:', pastedData.substring(0, 200));
                ...
                // ===== 2.4 WBET 格式检测和处理（优先检测，因为特征最明显：Sub Total/Grand Total） =====
                if (!formatDetected) {
                ...
                // ===== 2.5 ALIPAY 格式检测和处理（检测标识符行：2-10个大写字母） =====
                if (!formatDetected) {
                ...
                // ===== 2.6 PEGASUS 格式检测和处理（最后检测，合并所有数据成一行） =====
                if (!formatDetected) {
                    console.log('2.SPECIAL: Trying 2.6 PEGASUS format...');
                    console.log('2.SPECIAL: PEGASUS - Pasted data length:', pastedData.length);
                    console.log('2.SPECIAL: PEGASUS - Pasted data raw (first 500 chars):', pastedData.substring(0, 500));
