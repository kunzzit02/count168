    ...
    <script>
        ...
        function handleCellPaste(e) {
            ...
            if (typeof currentDataCaptureType !== 'undefined' && currentDataCaptureType === '2.SPECIAL') {
                ...
                // ===== 2.3 WBET 格式检测和处理 =====
                if (!formatDetected) {
                    console.log('2.SPECIAL: Trying 2.3 WBET format...');
                    ...
                    if (htmlDataFromDetect) {
                        ...
                        if (filled) {
                            console.log('2.SPECIAL: Detected WBET format (2.3) - HTML');
                            formatDetected = true;
                            showNotification('2.SPECIAL: 检测到WBET格式 (2.3)!', 'success');
                            setTimeout(updateSubmitButtonState, 0);
                            return;
                        } else {
                            console.log('2.SPECIAL: parseAndFillHTMLTableForWBET returned false, trying standard HTML parsing');
                        }
                    }
                    ...
                    if (htmlData && !formatDetected) {
                        console.log('2.SPECIAL: HTML data detected, length:', htmlData.length);
                        const filled = parseAndFillHTMLTableForWBET(htmlData, startCell);
                        if (filled) {
                            console.log('2.SPECIAL: Detected WBET format (2.3) - HTML manual');
                            formatDetected = true;
                            showNotification('2.SPECIAL: 检测到WBET格式 (2.3)!', 'success');
                            setTimeout(updateSubmitButtonState, 0);
                            return;
                    ...
                    if (!formatDetected) {
                        ...
                        // 快速检测：如果数据中包含 SUB TOTAL 或 GRAND TOTAL，很可能是 WBET 格式
                        const hasWbetMarkers = pastedData.toUpperCase().includes('SUB TOTAL') || 
                                              pastedData.toUpperCase().includes('SUBTOTAL') ||
                                              pastedData.toUpperCase().includes('GRAND TOTAL') || 
                                              pastedData.toUpperCase().includes('GRANDTOTAL');
                        
                        if (lines.length > 0 && hasWbetMarkers) {
                            // 第一步：解析原始数据成行
                            const rawDataMatrix = [];
                            lines.forEach(line => {
                                let cells = [];
                            ...
                            console.log('2.SPECIAL: Raw parsed data:', rawDataMatrix.length, 'rows');
                            ...
                            rawDataMatrix.forEach((row, rowIndex) => {
                                ...
                                if (isSubTotal || isGrandTotal) {
                                    ...
                                    console.log(`2.SPECIAL: ${isSubTotal ? 'SUB TOTAL' : 'GRAND TOTAL'} at row ${rowIndex}, next Total at row ${nextTotalRowIndex}`);
                                    ...
                                    while (mergeIndex < nextTotalRowIndex && mergeIndex < rawDataMatrix.length) {
                                        ...
                                        // 如果遇到另一个 Total 行，立即停止合并
                                        if (nextIsSubTotal || nextIsGrandTotal) {
                                            console.log(`2.SPECIAL: Stopping merge at row ${mergeIndex} - found another Total row`);
                                            break;
                                        }
                                        ...
                                        // 检查是否是用户名标识（2-3个大写字母）
                                        if (/^[A-Z]{2,3}$/.test(nextProcessedFirstCell)) {
                                            console.log(`2.SPECIAL: Stopping merge at row ${mergeIndex} - found new data row (${nextProcessedFirstCell})`);
                                            break; // 这是新的数据行，停止合并
                                        }
                                        ...
                                        if (processedRow.length > 0 && dataToAdd.length > 0) {
                                            ...
                                            if (lastValue && firstValue && lastValue.toString().trim() === firstValue.toString().trim()) {
                                                startIndex = 1; // 跳过第一个值（因为它是重复的）
                                                console.log(`2.SPECIAL: Text - Detected duplicate value "${firstValue}", skipping first cell of next row`);
                                            }
                                        }
                                        ...
                                        for (let i = startIndex; i < dataToAdd.length; i++) {
                                            ...
                                            if (cellValue) {
                                                ...
                                                if (lastProcessedValue && lastProcessedValue.toString().trim() === cellValue) {
                                                    // 如果与最后一个值相同，跳过（避免重复）
                                                    console.log(`2.SPECIAL: Text - Skipping duplicate value "${cellValue}" (same as last value)`);
                                                    continue;
                                                }
                                                ...
                                                if (processedRow.length >= 2) {
                                                    const secondLastValue = processedRow[processedRow.length - 2];
                                                    if (secondLastValue && secondLastValue.toString().trim() === cellValue) {
                                                        console.log(`2.SPECIAL: Text - Skipping duplicate value "${cellValue}" (same as second last value, pattern detected)`);
                                                        continue;
                                                    }
                                                }
                            ...
                            console.log(`2.SPECIAL: Found Sub Total at row ${subTotalRowIndex}, Grand Total at row ${grandTotalRowIndex}`);
                            ...
                            if (subTotalRowIndex >= 0 && grandTotalRowIndex >= 0 && grandTotalRowIndex > subTotalRowIndex) {
                                ...
                                console.log(`2.SPECIAL: Sub Total has ${subTotalDataCells.length} data cells, Grand Total has ${grandTotalDataCells.length} data cells`);
                                
                                // 根据用户需求：Sub Total 和 Grand Total 的数据应该是一样的
                                // 如果 Sub Total 行数据为空，而 Grand Total 行有数据，将 Grand Total 的数据复制到 Sub Total
                                if (subTotalDataCells.length === 0 && grandTotalDataCells.length > 0) {
                                    console.log('2.SPECIAL: Sub Total is empty but Grand Total has data. Copying Grand Total data to Sub Total.');
                                    const newSubTotalRow = ['SUB TOTAL', ...grandTotalDataCells];
                                    processedMatrix[subTotalRowIndex] = newSubTotalRow;
                                } else if (subTotalDataCells.length > 0 && grandTotalDataCells.length === 0) {
                                    console.log('2.SPECIAL: Grand Total is empty but Sub Total has data. Copying Sub Total data to Grand Total.');
                                    const newGrandTotalRow = ['GRAND TOTAL', ...subTotalDataCells];
                                    processedMatrix[grandTotalRowIndex] = newGrandTotalRow;
                                } else if (subTotalDataCells.length > 0 && grandTotalDataCells.length > 0) {
                                    // 两者都有数据，使用 Grand Total 的数据作为标准（因为通常 Grand Total 更完整）
                                    console.log('2.SPECIAL: Both have data. Ensuring Sub Total matches Grand Total.');
                                    const newSubTotalRow = ['SUB TOTAL', ...grandTotalDataCells];
                                    processedMatrix[subTotalRowIndex] = newSubTotalRow;
                                }
                            }
                            ...
                            const deduplicatedMatrix = finalMatrix.map((row, rowIdx) => {
                                ...
                                if (isSubTotal || isGrandTotal) {
                                    ...
                                    row.forEach((cell, cellIdx) => {
                                        ...
                                        if (cellIdx === 0 && (cellText.includes('SUB TOTAL') || cellText.includes('SUBTOTAL') || 
                                            ...
                                            if (lastValue === null || lastValue.toString().trim() !== cellValue) {
                                                ...
                                                console.log(`2.SPECIAL: Removing duplicate value "${cellValue}" at row ${rowIdx}, column ${cellIdx}`);
                                            }
                                        } else {
                                            // 空值也添加（保持列对齐）
                                            deduplicatedRow.push(cell);
                                    ...
                                    console.log(`2.SPECIAL: Row ${rowIdx} (${isSubTotal ? 'SUB TOTAL' : 'GRAND TOTAL'}): ${row.length} -> ${deduplicatedRow.length} cells after deduplication`);
                                    return deduplicatedRow;
                                }
                            ...
                            console.log('2.SPECIAL: Processed text data:', processedMatrix.length, 'rows x', maxCols, 'cols');
                            console.log('2.SPECIAL: First few processed rows:', processedMatrix.slice(0, 5));
                            ...
                            if (processedMatrix.length > 0) {
                                ...
                                if (successCount > 0) {
                                    console.log('2.SPECIAL: Detected WBET format (2.3)');
                                    formatDetected = true;
                                    showNotification(`2.SPECIAL: 检测到WBET格式 (2.3)，成功粘贴 ${successCount} 个单元格 (${processedMatrix.length} 行 x ${maxCols} 列)!`, 'success');
                                    setTimeout(updateSubmitButtonState, 0);
                                    return;
                                }
                            }
                    ...
                    // WBET 解析失败，继续尝试其他格式
                    if (!formatDetected) {
                        console.log('2.SPECIAL: WBET parser failed, continuing with other formats');
                    }
                }
                ...
                if (!formatDetected) {
                    ...
                    if (htmlDataFromDetect) {
                        ...
                        if (filled) {
                            ...
                            console.log('2.SPECIAL: parseAndFillHTMLTable returned false, trying manual HTML parsing');
                        }
                    }
                    ...
                    if (htmlData && !formatDetected) {
                        ...
                        try {
                            ...
                            const table = tempDiv.querySelector('table');
                            if (table) {
                                console.log('2.SPECIAL: HTML table found');
                                let dataMatrix = [];
                                ...
                                if (dataMatrix.length > 0) {
                                    ...
                                    console.log('2.SPECIAL: HTML parsing successful -', dataMatrix.length, 'rows x', maxCols, 'cols');
                                    ...
                                    console.log('2.SPECIAL: HTML table found but no data rows extracted');
                                }
                            } else {
                                console.log('2.SPECIAL: HTML data exists but no table element found');
                            }
                        } catch (htmlErr) {
                            console.error('2.SPECIAL: HTML parser error:', htmlErr);
                        }
                        ...
                        if (lines.length > 0) {
                            ...
                            if (lines.length >= 3) {
                                ...
                                if (identifierCount >= 2 && nameLikeLineCount >= identifierCount * 0.5) {
                                    ...
                                    console.log('2.SPECIAL: Detected Name column format (', nameLikeLineCount, 'out of', identifierCount, 'identifiers)');
                                }
                            }
                                                    ...
                                                    if (isNameLike) {
                                                        ...
                                                        if (i + 2 < lines.length) {
                                                            ...
                                                            if (hasNumbers) {
                                                                ...
                                                                // 跳过 Name 行的处理
                                                                i++; // 跳过下一行（Name 行）
                                                                console.log('2.SPECIAL: Detected Name column value:', nameValue, 'for identifier:', trimmedLine);
                                                            }
                                                        }
                                                    }
                                                }
                            ...
                            if (dataMatrix.length > 0) {
                                console.log('2.SPECIAL: Text parsing successful -', dataMatrix.length, 'rows x', maxCols, 'cols');
                                console.log('2.SPECIAL: First row sample:', dataMatrix[0] ? dataMatrix[0].slice(0, 10) : 'empty');
                                alipayParsed = {
                                    dataMatrix: dataMatrix,
                                    maxRows: dataMatrix.length,
                                    maxCols: maxCols
                    ...
                    if (alipayParsed && !formatDetected) {
                        ...
                        if (successCount > 0) {
                            console.log('2.SPECIAL: Detected ALIPAY format (2.5)');
                            formatDetected = true;
